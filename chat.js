...
import { renderPollMessage, renderGifMessage, addMessageStats, renderThreadCount } from "./extras.js";
...
function appendMessage(msg, container) {
  ...
  const frameStyle = msg.avatarFrame || '';
  ...
  div.innerHTML = `
    <div class="msg-avatar ${frameStyle}" style="background:${color}" data-uid="${msg.authorId || ''}" title="${escHtml(msg.author || '')}">${initial}</div>
    <div class="msg-body">
  ...
  // Special message types rendered inline
  const contentEl = div.querySelector(`#mc-${msg.id}`);
  if (msg.type === 'poll' && contentEl) {
    contentEl.innerHTML = '';
    contentEl.appendChild(renderPollMessage(msg));
  } else if (msg.type === 'gif' && contentEl) {
    contentEl.innerHTML = '';
    contentEl.appendChild(renderGifMessage(msg));
  } else if (contentEl) {
    addMessageStats(contentEl, msg.content);
    const tc = renderThreadCount(msg);
    if (tc) contentEl.appendChild(tc);
  }
...
async function sendMessage() {
  ...
  const msgData = {
    ...
    avatarFrame: state.userDoc?.profile?.frameId || '',
