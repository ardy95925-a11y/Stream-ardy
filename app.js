...
const ICE_CONFIG = {
  iceServers: [
    ...
    // Free public TURN servers — critical fallback for mobile/NAT networks
    // where STUN alone can't establish a direct peer-to-peer path.
    {
      urls: [
...
function resetOverlayUI(peerData, type) {
  ...
  // Reset remote stream accumulator so the next call gets a fresh stream
  _remoteStream = null;
...
//
// FIX: We can no longer rely on event.streams[0] — Firefox and many mobile
// browsers deliver tracks with an empty streams array.  Instead we maintain
// a single shared MediaStream (`_remoteStream`) and add every incoming track
// to it manually.  Both the <audio> and <video> elements are always pointed
// at this one stream so they automatically pick up whichever tracks arrive
// and in whichever order they arrive.
let _remoteStream = null;

function handleRemoteTrack(event, type) {
  // Lazily create (or reuse) our shared remote MediaStream.
  if (!_remoteStream) {
    // Prefer event.streams[0] if the browser supplied it; otherwise make our own.
    _remoteStream = (event.streams && event.streams[0]) ? event.streams[0] : new MediaStream();
  }

  // If the browser did NOT attach the track to a stream, add it ourselves.
  if (!event.streams || !event.streams[0]) {
    _remoteStream.addTrack(event.track);
  }

  // ── Audio wiring ───────────────────────────────────────
  // Always wire the full stream to the <audio> element; it only plays audio tracks.
  const audioEl = $('remoteAudio');
  if (audioEl && audioEl.srcObject !== _remoteStream) {
    audioEl.srcObject = _remoteStream;
  }
  if (audioEl) {
    // Call play() every time a track arrives — it is idempotent when already playing
    // and is required on some browsers after srcObject is set programmatically.
    audioEl.play().catch(err => {
      // Any autoplay block (NotAllowedError or otherwise) — resume on next tap.
  ...
  // ── Video wiring (video calls only) ───────────────────
  if (type === 'video') {
    const rv = $('remoteVideo');
    if (rv) {
      if (rv.srcObject !== _remoteStream) rv.srcObject = _remoteStream;
      rv.style.display = '';
      // Explicitly call play() — required on mobile after srcObject assignment.
      rv.play().catch(() => {});
...
async function endCall() {
  ...
  if (callDocId) {
    const docIdToEnd = callDocId;
    callDocId = null;
    updateDoc(doc(db, 'calls', docIdToEnd), { status: 'ended' }).catch(() => {});
  }

  // Close peer connection
  if (pc) { pc.close(); pc = null; }

  // Reset remote stream accumulator for the next call
  _remoteStream = null;
  ...
  if (_callEmojiPickerClickHandler) {
    document.removeEventListener('click', _callEmojiPickerClickHandler);
    _callEmojiPickerClickHandler = null;
  }
...
let _callEmojiPickerClickHandler = null; // stored so we can remove it
...
function wireCallButtons() {
  ...
  // Remove previous click handler before adding new one
  if (_callEmojiPickerClickHandler) document.removeEventListener('click', _callEmojiPickerClickHandler);
  _callEmojiPickerClickHandler = e => {
    if (!e.target.closest('#callEmojiPicker') && !e.target.closest('#callReactBtn')) showEmojiPicker(false);
  };
  document.addEventListener('click', _callEmojiPickerClickHandler);
  ...
  try {
    ...
    callUnsub = onSnapshot(callRef, async snap => {
      ...
      if (data.answer && pc && pc.signalingState === 'have-local-offer') {
        ...
        if (!calleeCandUnsub) {
          const thisCallId = callRef.id;
          calleeCandUnsub = onSnapshot(
            collection(db, 'calls', thisCallId, 'calleeCandidates'),
            s => {
      ...
      if (data.status === 'ended' && callDocId === callRef.id) {
        endCall();
      }
    });
...
async function answerCall(callerData, callId, callData) {
  ...
  try {
    ...
    callUnsub = onSnapshot(doc(db, 'calls', callId), snap => {
      const data = snap.data();
      if (data?.status === 'ended' && callDocId === callId) endCall();
    });
  ...
  onSnapshot(q, async snap => {
    for (const change of snap.docChanges()) {
      if (change.type === 'added') {
        ...
        // Ignore calls older than 60 seconds (stale/missed calls)
