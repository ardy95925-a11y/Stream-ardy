...
function loadSidebar() {
  ...
  // Profile buttons
  ['open-profile-btn', 'footer-profile-btn', 'sidebar-settings-btn'].forEach(id => {
...
export function showUserPopup(user) {
  const headerEl = document.getElementById('popup-header-color');
  const avEl     = document.getElementById('popup-avatar');
  const nmEl     = document.getElementById('popup-display-name');
  const unEl     = document.getElementById('popup-username');
  const stEl     = document.getElementById('popup-status-line');
  const bioEl    = document.getElementById('popup-bio');
  const dmBtn    = document.getElementById('popup-dm-btn');

  if (headerEl) headerEl.style.background = `linear-gradient(135deg, ${user.avatarColor || '#FF6B1A'}, rgba(0,0,0,0.5))`;
  if (avEl)   { avEl.textContent = (user.displayName || '?').charAt(0).toUpperCase(); avEl.style.background = user.avatarColor || '#FF6B1A'; }
  if (nmEl)   nmEl.textContent = user.displayName || 'Unknown';
  if (unEl)   unEl.textContent = `@${user.username || ''}`;
  if (stEl)   stEl.innerHTML   = `<span style="color:${statusColor(user.status)}">● ${(user.status || 'offline').charAt(0).toUpperCase() + (user.status || 'offline').slice(1)}</span>`;
  if (bioEl)  bioEl.textContent = user.bio || 'No bio yet.';
  if (dmBtn)  {
    dmBtn.dataset.uid = user.uid;
    dmBtn.style.display = user.uid === state.user?.uid ? 'none' : '';
  }
  showModal('user-popup-modal');
}
