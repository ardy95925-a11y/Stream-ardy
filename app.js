...
async function showCallModal(type) {
  ...
  const videoArea = $('callVideoArea');
  const camCtrl = $('callCamCtrl');

  setAvatar($('callAvatar'), currentFriendData);
  $('callName').textContent = currentFriendData.displayName;
  $('callStatusText').textContent = (type === 'video' ? '📹 Video' : '📞 Voice') + ' calling…';
  $('callStatusDot').classList.remove('active');
  $('callDuration').classList.remove('active');

  if (videoArea) { videoArea.classList.toggle('active', type === 'video'); }
  if (camCtrl) { camCtrl.style.display = type === 'video' ? '' : 'none'; }

  overlay.classList.remove('hidden');

  // Animated dots while calling
  let dots = 0;
  const iv = setInterval(() => {
    dots = (dots + 1) % 4;
    const txt = (type === 'video' ? '📹 Video' : '📞 Voice') + ' calling' + '.'.repeat(dots);
    const el = $('callStatusText');
  ...
  function startDuration() {
    $('callDuration').classList.add('active');
    $('callStatusDot').classList.add('active');
    $('callStatus').style.display = 'none';
    durationIv = setInterval(() => {
      durationSec++;
      const m = Math.floor(durationSec / 60);
      const s = durationSec % 60;
      const dur = $('callDuration');
      if (dur) dur.textContent = `${m}:${s.toString().padStart(2,'0')}`;
    }, 1000);
  }
  ...
  try {
    ...
    if (localVideo && type === 'video') localVideo.srcObject = localStream;
    ...
    peerConnection.ontrack = (event) => {
      if (remoteVideo) {
        ...
        if (statusEl) statusEl.textContent = type === 'video' ? '📹 Connected' : '📞 Connected';
        startDuration();
      }
    };
    ...
    peerConnection.oniceconnectionstatechange = () => {
      if (peerConnection?.iceConnectionState === 'disconnected' || peerConnection?.iceConnectionState === 'failed') {
        clearInterval(iv); clearInterval(durationIv);
        endCall();
    ...
    peerConnection.onicecandidate = async (event) => {
      if (event.candidate) {
        await addDoc(collection(db, 'calls', callDocId, 'callerCandidates'), event.candidate.toJSON()).catch(() => {});
      }
    };
        ...
        onSnapshot(collection(db, 'calls', callDocId, 'calleeCandidates'), (s) => {
          s.docChanges().forEach(async change => {
            if (change.type === 'added') {
              await peerConnection?.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(() => {});
            }
          });
        });
      }
      if (data?.status === 'ended') { clearInterval(iv); clearInterval(durationIv); endCall(); }
    });
    ...
    if (err.name === 'NotAllowedError' || err.name === 'NotFoundError') {
      ...
      const statusEl = $('callStatusText');
      if (statusEl) statusEl.textContent = (type === 'video' ? '📹 Video' : '📞 Voice') + ' call (no mic/cam)';
    } else {
  ...
  // End call button
  $('endCallBtn').onclick = () => { clearInterval(iv); clearInterval(durationIv); endCall(); toast('Call ended'); };
    ...
    camBtn.onclick = () => {
      if (localStream) {
        ...
        if (videoTrack) {
          ...
          camBtn.classList.toggle('on', off);
          const lbl = $('camBtnLabel');
          if (lbl) lbl.textContent = off ? 'Cam Off' : 'Camera';
...
function showIncomingCallUI(caller, callId, callData) {
  ...
  setAvatar($('incomingAvatar'), caller);
  $('incomingCallerName').textContent = caller.displayName;
  $('incomingCallType').textContent = callData.type === 'video' ? '📹 Incoming video call' : '📞 Incoming voice call';

  playNotifSound();

  const declineBtn = $('declineCallBtn');
  const answerBtn  = $('answerCallBtn');

  // Clone to remove old listeners
  const newDecline = declineBtn.cloneNode(true);
  const newAnswer  = answerBtn.cloneNode(true);
  declineBtn.parentNode.replaceChild(newDecline, declineBtn);
  answerBtn.parentNode.replaceChild(newAnswer, answerBtn);
  ...
  setTimeout(() => {
    if (!$('incomingCallOverlay').classList.contains('hidden')) {
      $('incomingCallOverlay').classList.add('hidden');
...
async function answerCall(caller, callId, callData) {
  ...
  setAvatar($('callAvatar'), caller);
  $('callName').textContent = caller.displayName;
  $('callStatusText').textContent = callData.type === 'video' ? '📹 Connecting…' : '📞 Connecting…';
  $('callStatusDot').classList.remove('active');
  $('callDuration').classList.remove('active');
  if (videoArea) videoArea.classList.toggle('active', callData.type === 'video');
  if (camCtrl) camCtrl.style.display = callData.type === 'video' ? '' : 'none';
  ...
  try {
    ...
    peerConnection.ontrack = (event) => {
      if (remoteVideo) {
        remoteVideo.srcObject = event.streams[0];
        const statusEl = $('callStatusText');
        if (statusEl) statusEl.textContent = callData.type === 'video' ? '📹 Connected' : '📞 Connected';
        startDuration();
      }
    };
    ...
    peerConnection.onicecandidate = async (event) => {
      if (event.candidate) {
        await addDoc(collection(db, 'calls', callId, 'calleeCandidates'), event.candidate.toJSON()).catch(() => {});
      }
    };
    ...
    // Also listen for status: ended
    callUnsub = onSnapshot(doc(db, 'calls', callId), snap => {
      if (snap.data()?.status === 'ended') { clearInterval(durationIv); endCall(); }
    });
  ...
  $('endCallBtn').onclick = () => { clearInterval(durationIv); endCall(); toast('Call ended'); };
...
// Escape on call overlay = end call
document.addEventListener('keydown', e => {
  if (e.key === 'Escape' && !$('callOverlay').classList.contains('hidden')) {
