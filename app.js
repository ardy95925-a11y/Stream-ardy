// =====================================================
//  PULSE — app.js  v6  (ES Module, Firebase v10)
...
function initApp() {
  ...
  // FIX C1: Expose sendMessage for media.js
  window.__pulseSendMessage = sendMessage;
}
...
function subscribeToMessages(chatId) {
  ...
  messagesUnsub = onSnapshot(q, snap => {
    ...
    if (!firstLoad && lastMsg && lastMsg.senderId !== currentUser.uid) playNotifSound();
    firstLoad = false;
    renderMessages(msgs);
    if (!document.hidden && currentChatId === chatId) markMessagesRead(chatId, msgs);
  });
}
...
function renderMessages(messages) {
  ...
  for (const msg of messages) {
    ...
    const showCtxMenu = (e) => { e.preventDefault(); showMessageContextMenu(e, msg, isMine); };
...
async function sendMessage(payload) {
  // FIX C1: Always keep this global reference current for media.js
  window.__pulseSendMessage = sendMessage;
  if (!currentChatId) throw new Error('No active chat — open a conversation first');
  const msg = {
  ...
  try {
    ...
    batch.update(doc(db, 'chats', currentChatId), {
      lastMessage: preview, lastMessageTime: serverTimestamp(),
...
async function loadGifs(searchQuery) {
  ...
  try {
    ...
    grid.innerHTML = `<div class="gif-no-key">Couldn't load GIFs.</div>`;
  }
}
...
function showChatPanel(show) {
  ...
  show ? p.classList.remove('call-chat-panel-hidden') : p.classList.add('call-chat-panel-hidden');
}

function showEmojiPicker(show) {
  const p = $('callEmojiPicker'); if (!p) return;
  show ? p.classList.remove('call-emoji-picker-hidden') : p.classList.add('call-emoji-picker-hidden');
}

function startCallTimer() {
  if (callStartedAt) return; // already running
  callStartedAt = Date.now();
  const el = $('callTimer'); if (el) el.style.display = '';
  callTimerInt = setInterval(() => {
    if (!callStartedAt) return;
    const s = Math.floor((Date.now() - callStartedAt) / 1000);
    const m = Math.floor(s / 60);
    const sec = (s % 60).toString().padStart(2, '0');
    const el2 = $('callTimer'); if (el2) el2.textContent = `${m}:${sec}`;
  }, 1000);
}
...
function stopCallTimer() {
  ...
  callStartedAt = null;
  const el = $('callTimer'); if (el) { el.style.display = 'none'; el.textContent = '0:00'; }
}

let bgAnimId = null;
function startBgAnim() {
  const canvas = $('callBgCanvas'); if (!canvas) return null;
  const ctx = canvas.getContext('2d');
  function resize() { canvas.width = window.innerWidth; canvas.height = window.innerHeight; }
  resize(); window.addEventListener('resize', resize);
  const pts = Array.from({length:30}, () => ({
    x: Math.random()*window.innerWidth, y: Math.random()*window.innerHeight,
    r: Math.random()*1.6+0.4, dx:(Math.random()-0.5)*0.3, dy:(Math.random()-0.5)*0.3, a:Math.random()*0.35+0.08
  }));
  function draw() {
    ...
    g.addColorStop(0,'rgba(255,107,53,0.07)'); g.addColorStop(1,'transparent');
  ...
  draw();
  return () => { cancelAnimationFrame(bgAnimId); window.removeEventListener('resize', resize); };
}
let stopBgAnim = null;
...
function makePipDraggable() {
  ...
  const down=(cx,cy)=>{ drag=true; const r=pip.getBoundingClientRect(); ox=cx-r.left; oy=cy-r.top; };
  const move=(cx,cy)=>{ if(!drag)return; let nx=cx-ox,ny=cy-oy; nx=Math.max(8,Math.min(window.innerWidth-pip.offsetWidth-8,nx)); ny=Math.max(70,Math.min(window.innerHeight-pip.offsetHeight-150,ny)); pip.style.left=nx+'px'; pip.style.top=ny+'px'; pip.style.right='auto'; };
  const up=()=>{ drag=false; };
...
function resetOverlay(peerData, type) {
  ...
  resetMuteBtn(); resetCamBtn();
  const camItem = $('cameraCtrlItem'); if (camItem) camItem.style.display = type==='video' ? 'flex':'none';
  // Reset all video/audio elements
  ['remoteAudio','localVideo','remoteVideo','screenShareVideo'].forEach(id => {
    const el = $(id); if (!el) return;
    el.srcObject = null;
    if (id !== 'remoteAudio' && id !== 'localVideo') el.style.display = 'none';
  });
  const pip = $('localVideoPip'); if (pip) pip.style.display = 'none';
  const si = $('screenShareIndicator'); if (si) si.style.display = 'none';
  $('callVoiceCenter').style.display = '';
  showChatPanel(false); showEmojiPicker(false);
  $('callChatMessages').innerHTML = '';
  $('callReactLayer').innerHTML = '';
  applyBetaUI();
  if (stopBgAnim) { stopBgAnim(); stopBgAnim = null; }
  stopBgAnim = startBgAnim();
  makePipDraggable();
}

// FIX C3/C12: Route remote tracks — audio always to <audio>, video to <video>
function handleRemoteTrack(event, type) {
      ...
      if (p !== undefined) {
        p.catch(err => {
          if (err.name === 'NotAllowedError') {
            // Autoplay blocked — attach a one-time gesture handler
  ...
  startCallTimer(); // FIX C4: start timer on connection
}

// ── START OUTGOING CALL ──
async function startCall(type) {
  // FIX C7: End any active call first
  if (peerConnection || callDocId) await endCall();

  callType = type;
  resetOverlay(currentFriendData, type);
  openCallOverlay();
  ...
  try {
    ...
    if (type === 'video') {
      ...
      const pip = $('localVideoPip'); if (pip) pip.style.display = '';
    }

    peerConnection = new RTCPeerConnection(ICE_SERVERS);
    localStream.getTracks().forEach(t => peerConnection.addTrack(t, localStream));

    peerConnection.ontrack = (ev) => { clearInterval(dotInterval); handleRemoteTrack(ev, type); };
    peerConnection.oniceconnectionstatechange = () => {
      const s = peerConnection?.iceConnectionState;
      if (s === 'disconnected' || s === 'failed') { endCall(); toast('Call disconnected', 'error'); }
    };
    ...
    peerConnection.onicecandidate = async ev => {
      if (ev.candidate) await addDoc(collection(db,'calls',callDocId,'callerCandidates'), ev.candidate.toJSON()).catch(()=>{});
    };
    ...
    await setDoc(callRef, {
      ...
      callerId: currentUser.uid, calleeId: currentFriendId,
      type, status: 'calling', createdAt: serverTimestamp()
    });

    let calleeCandListenerSet = false;
    ...
    callUnsub = onSnapshot(callRef, async snap => {
      ...
      if (data.answer && peerConnection && peerConnection.signalingState !== 'stable') {
        ...
        try { await peerConnection.setRemoteDescription(new RTCSessionDescription(data.answer)); } catch(e) {}

        // FIX C6: subscribe callee candidates only once
        if (!calleeCandListenerSet) {
          calleeCandListenerSet = true;
          callCandUnsub = onSnapshot(collection(db,'calls',callDocId,'calleeCandidates'), s => {
            s.docChanges().forEach(async ch => {
              if (ch.type === 'added' && peerConnection) {
                await peerConnection.addIceCandidate(new RTCIceCandidate(ch.doc.data())).catch(()=>{});
              }
            });
          });
        }
      ...
      // FIX C4: also start timer when status changes to connected
      if (data.status === 'connected') { clearInterval(dotInterval); startCallTimer(); }
      if (data.status === 'ended')     { clearInterval(dotInterval); endCall(); }
    });

  } catch(err) {
    clearInterval(dotInterval);
    toast(err.name === 'NotAllowedError' ? 'Mic/camera access denied' : 'Could not start call: ' + err.message, 'error');
    // cleanup on error
    if (callDocId) { updateDoc(doc(db,'calls',callDocId), {status:'ended'}).catch(()=>{}); callDocId = null; }
  ...
  wireCallButtons(dotInterval);
}

// ── ANSWER INCOMING CALL ──
async function answerCall(caller, callId, callData) {
  // FIX C7: End any active call first
  if (peerConnection || callDocId) await endCall();

  callType = callData.type;
  resetOverlay(caller, callData.type);
  openCallOverlay();
  ...
  try {
    ...
    peerConnection = new RTCPeerConnection(ICE_SERVERS);
    localStream.getTracks().forEach(t => peerConnection.addTrack(t, localStream));
    peerConnection.ontrack = (ev) => handleRemoteTrack(ev, callData.type);
    peerConnection.oniceconnectionstatechange = () => {
      const s = peerConnection?.iceConnectionState;
      if (s === 'disconnected' || s === 'failed') { endCall(); toast('Call disconnected', 'error'); }
    };
    ...
    peerConnection.onicecandidate = async ev => {
      if (ev.candidate) await addDoc(collection(db,'calls',callId,'calleeCandidates'), ev.candidate.toJSON()).catch(()=>{});
    };

    await peerConnection.setRemoteDescription(new RTCSessionDescription(callData.offer));

    // FIX C2: Subscribe to CALLER candidates (bug was subscribing to calleeCandidates)
    callCandUnsub = onSnapshot(collection(db,'calls',callId,'callerCandidates'), s => {
      s.docChanges().forEach(async ch => {
        if (ch.type === 'added' && peerConnection) {
          await peerConnection.addIceCandidate(new RTCIceCandidate(ch.doc.data())).catch(()=>{});
        }
    ...
    // FIX C4: Start timer immediately on answer
    startCallTimer();
  ...
  wireCallButtons(null);
}

// ── WIRE BUTTONS ── FIX C8: clone nodes to remove old listeners
function wireCallButtons(dotInterval) {
  ...
  rewire('muteBtn', () => {
    ...
    resetMuteBtn(); toast(isMuted ? 'Muted' : 'Unmuted');
  });

  const camBtnEl = $('cameraBtn');
  if (camBtnEl) {
    const camClone = camBtnEl.cloneNode(true);
    camBtnEl.parentNode.replaceChild(camClone, camBtnEl);
    camClone.onclick = () => {
      if (!localStream) return;
      const track = localStream.getVideoTracks()[0]; if (!track) return;
      isCamOff = !isCamOff; track.enabled = !isCamOff;
      resetCamBtn(); toast(isCamOff ? 'Camera off' : 'Camera on');
    };
  }
  ...
  const closeChatBtn = $('closeCallChatBtn');
  if (closeChatBtn) closeChatBtn.onclick = () => showChatPanel(false);

  const sendMsg = () => {
    const inp = $('callChatInput'); const txt = inp?.value.trim(); if (!txt) return;
    appendCallChatMsg('You', txt); inp.value = '';
  };
  const chatSendBtn = $('callChatSendBtn'); if (chatSendBtn) chatSendBtn.onclick = sendMsg;
  const chatInput = $('callChatInput'); if (chatInput) chatInput.onkeydown = e => { if(e.key==='Enter') sendMsg(); };

  const ssBtn = $('screenShareBtn');
  if (ssBtn) {
    ssBtn.onclick = async () => {
      if (!betaEnabled) { toast('Enable Beta Features in Settings first', 'error'); return; }
      isScreenSharing ? await stopScreenShare() : await beginScreenShare();
    };
  }
...
function resetMuteBtn() {
  const btn=$('muteBtn'), icon=$('muteBtnIcon'), lbl=$('muteLbl'); if (!btn) return;
  if (isMuted) {
    btn.classList.add('call-btn-off'); if(lbl)lbl.textContent='Unmute';
    ...
    btn.classList.remove('call-btn-off'); if(lbl)lbl.textContent='Mute';
    if(icon)icon.innerHTML=`<path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/>`;
  }
}

function resetCamBtn() {
  const btn=$('cameraBtn'); if(!btn)return; btn.classList.toggle('call-btn-off',isCamOff);
}

function appendCallChatMsg(who, text) {
  const box=$('callChatMessages'); if(!box)return;
  const el=document.createElement('div'); el.className='call-chat-msg';
  el.innerHTML=`<span class="ccm-who">${escHtml(who)}</span><span class="ccm-text">${escHtml(text)}</span>`;
  box.appendChild(el); box.scrollTop=99999;
...
function sendFloatEmoji(emoji) {
  ...
  const el=document.createElement('div'); el.className='call-float-emoji';
  el.textContent=emoji; el.style.left=(10+Math.random()*70)+'%';
...
async function beginScreenShare() {
  try {
    ...
    const sv=$('screenShareVideo'); if(sv){sv.srcObject=screenStream;sv.style.display='';sv.play().catch(()=>{});}
    const si=$('screenShareIndicator'); if(si)si.style.display='flex';
    isScreenSharing=true;
    const ssBtn=$('screenShareBtn'); if(ssBtn)ssBtn.classList.add('call-btn-active');
...
async function stopScreenShare() {
  ...
  const sv=$('screenShareVideo'); if(sv){sv.style.display='none';sv.srcObject=null;}
  const si=$('screenShareIndicator'); if(si)si.style.display='none';
  if (peerConnection && localStream && callType==='video') {
...
function listenForIncomingCalls() {
  const q = query(collection(db,'calls'), where('calleeId','==',currentUser.uid), where('status','==','calling'));
  onSnapshot(q, async snap => {
    for (const change of snap.docChanges()) {
      if (change.type === 'added') {
        const data = change.doc.data();
        if (data.callerId === currentUser.uid) continue; // don't answer our own calls
...
function showIncomingCallUI(caller, callId, callData) {
  ...
  const avEl = document.createElement('div'); avEl.className='avatar md';
  setAvatar(avEl, caller);
  const nameEl = document.createElement('div');
  nameEl.innerHTML = `<div style="font-weight:600;font-size:14px">${escHtml(caller.displayName)}</div><div style="font-size:12px;color:var(--text-muted)">${callData.type==='video'?'📹 Video':'📞 Voice'} call incoming…</div>`;
  const answerBtn = document.createElement('button');
  answerBtn.className = 'btn btn-primary btn-accept-call';
  answerBtn.style.cssText = 'padding:8px 16px;font-size:13px';
  answerBtn.textContent = 'Answer';
  const declineBtn = document.createElement('button');
  declineBtn.className = 'btn btn-ghost btn-decline-call';
  declineBtn.style.cssText = 'padding:8px 16px;font-size:13px;color:#ff4757';
  declineBtn.textContent = 'Decline';

  const left = document.createElement('div');
  left.style.cssText = 'display:flex;align-items:center;gap:12px;flex:1';
  left.appendChild(avEl); left.appendChild(nameEl);
  banner.appendChild(left); banner.appendChild(answerBtn); banner.appendChild(declineBtn);
...
function openProfileModal(userData, uid, isFriend) {
  ...
  const bioEl = $('profileBio'); if (bioEl) bioEl.textContent = userData.bio || '';
  const statusEl = $('profileStatusMsg');
  if (statusEl) {
    if (userData.statusMsg) { statusEl.style.display=''; statusEl.textContent=(userData.moodEmoji||'💬')+' '+userData.statusMsg; }
  ...
  if (badge) {
    ...
    badge.textContent = ageMonths > 12 ? '⭐ Veteran' : ageMonths > 3 ? '🌱 Regular' : '✨ New';
  }
  ...
  if (isFriend) {
    ...
    // FIX C5: Use startCall() — showCallModal() doesn't exist
    callBtn.addEventListener('click', () => {
      hideModal('profileModal');
      if (currentFriendId !== uid) { currentFriendId = uid; currentFriendData = userData; }
      startCall('voice');
    });
...
function startProfileParticles(style) {
  ...
  const canvas = $('profileParticles'); if (!canvas) return;
  const ctx = canvas.getContext('2d');
  canvas.width = canvas.parentElement.offsetWidth || 360;
  canvas.height = canvas.parentElement.offsetHeight || 200;
  const COLORS = {
    flame:['#ff6b35','#ffbe0b','#ff4500','#ffd700'],ocean:['#00b4d8','#90e0ef','#caf0f8','#48cae4'],
    galaxy:['#c77dff','#9d4edd','#e0aaff','#fff'],aurora:['#00b09b','#96c93d','#f7971e','#7bed9f'],
    neon:['#00f0ff','#ff00ff','#fff','#00ff88'],sakura:['#ff758c','#ff7eb3','#ffd6e7','#fff'],
    midnight:['#4040b0','#6060d0','#8080f0','#c0c0ff'],gold:['#ffd700','#ffb347','#fffacd','#daa520'],
    emerald:['#2ecc71','#00704a','#a8f0c6','#00ff88'],crimson:['#dc143c','#ff6b6b','#ff9999','#fff'],
    ice:['#00bcd4','#e0f7fa','#80deea','#fff'],sunset:['#ff9800','#ffd54f','#e65100','#ffcc02'],
  };
  const colors = COLORS[style] || COLORS.flame;
  const particles = Array.from({length:30}, () => ({
    x:Math.random()*canvas.width,y:Math.random()*canvas.height+canvas.height,
    r:Math.random()*3+1,speed:Math.random()*0.8+0.3,
    color:colors[Math.floor(Math.random()*colors.length)],
    opacity:Math.random()*0.7+0.2,drift:(Math.random()-0.5)*0.5,
  }));
  function animate() {
    ctx.clearRect(0,0,canvas.width,canvas.height);
    particles.forEach(p => {
...
$('openSettingsBtn').addEventListener('click', () => {
  ...
  const soundBtn = $('soundToggleBtn'); if (soundBtn) soundBtn.textContent = soundEnabled ? 'On ✓' : 'Off';
...
document.querySelectorAll('.modal-backdrop').forEach(b => {
  b.addEventListener('click', e => {
    if (e.target === b && b.id !== 'setupModal') b.classList.add('hidden');
  });
});
