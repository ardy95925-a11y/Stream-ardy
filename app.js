...
async function saveProfile() {
  ...
  const pronouns    = $('setupPronouns')?.value.trim() || '';
...
// ── Init ──────────────────────────────────────────────
function initApp() {
  // Expose sendMessage bridge for media.js (must happen before anything that media.js might call)
  window.__pulseSendMessage = (payload) => sendMessage(payload);
  window.__pulseSendMessageDirect = (payload) => sendMessage(payload);
  ...
  setupEmojiHints();
  // Show flip button on iPad/mobile (touch device)
  if (navigator.maxTouchPoints > 0) {
    const fc = $('callFlipCtrl'); if (fc) fc.style.display = 'flex';
  }
  // Enhanced message search (new feature 2)
  window.__enhanceMessageSearch && window.__enhanceMessageSearch();
  ...
  // New Feature 2: enhanced search nav (runs after DOM ready)
  setTimeout(() => window.__enhanceMessageSearch?.(), 200);
}
...
function renderChatItem(container, chatId, chatData, userData, friendUid) {
  ...
  if (chatData.lastMessageType === 'video') preview = '📹 Video';
...
function renderMessages(messages) {
  ...
  for (const msg of messages) {
    ...
    if (msg.type === 'image' || msg.type === 'gif') {
      ...
      video.style.cssText = 'max-width:280px;max-height:320px;border-radius:12px;display:block;background:#000';
      const caption = msg.text && msg.text !== '📹 Video' ? document.createElement('div') : null;
      if (caption) { caption.style.cssText = 'font-size:12px;color:var(--text-muted);padding:4px 4px 0'; caption.textContent = msg.text; }
  ...
  if (messages.length > 0) {
    ...
    if (isMine) {
      ...
      // New Feature 3: pulse animation when message is newly seen
      if (last.read) window.__applySeenPulse?.(meta, true);
    }
...
function setupInputBar() {
  ...
  $('msgSearchCloseBtn')?.addEventListener('click', () => { $('msgSearchBar').classList.add('hidden'); if (searchMode) toggleMessageSearch(); });
...
// ═══════════════════════════════════════════════════════════
//  PULSE — CALL SYSTEM v4  (Full rewrite, iPad Safari first)
//
//  Design goals:
//    • Works on iPad Safari 15+ without any hacks or fallbacks
//    • Voice calls  — crystal clear, no dropped audio
//    • Video calls  — full-screen remote, draggable local PiP
//    • In-call chat — live text during the call
//    • Emoji react  — floating emoji burst reactions
//    • TURN relay   — works behind every NAT/firewall/mobile network
//    • Zero screen-share complexity removed entirely
//
//  Safari-specific decisions:
//    • getUserMedia called DIRECTLY inside a tap handler (no await gap before it)
//    • .play() always called explicitly after srcObject assignment
//    • MediaStream tracks accumulated manually (streams[0] unreliable on Safari)
//    • No getDisplayMedia (screen share removed)
//    • iceCandidatePoolSize pre-warms candidates
// ═══════════════════════════════════════════════════════════

// ── Header call buttons ──────────────────────────────────
$('callBtn').addEventListener('click',  () => { if (currentFriendData) _initiateCall('voice'); });
$('videoBtn').addEventListener('click', () => { if (currentFriendData) _initiateCall('video'); });

// ── ICE config — STUN + free public TURN for relay ───────
const ICE_CONFIG = {
  iceServers: [
    { urls: 'stun:stun.l.google.com:19302' },
    { urls: 'stun:stun1.l.google.com:19302' },
    { urls: 'stun:stun2.l.google.com:19302' },
    // TURN relay — essential for mobile networks & symmetric NATs.
    // Without TURN, roughly 20% of real-world calls fail silently.
    {
      urls: [
...
let _localStream    = null;   // local mic/camera MediaStream
let _remoteStream   = null;   // accumulated remote MediaStream
let _callDocId      = null;   // Firestore call document ID
let _callType       = null;   // 'voice' | 'video'
let _callRole       = null;   // 'caller' | 'callee'
let _callUnsub      = null;   // Firestore snapshot unsubscribe
let _callerCandUnsub = null;
let _calleeCandUnsub = null;
let _timerInt       = null;
...
// ── Safari: getUserMedia must be called synchronously in a
//    tap handler. We store the call parameters and trigger
//    getUserMedia from within the button handler itself.
//    This avoids the "user gesture has been consumed" error.

function _initiateCall(type) {
  // Called from a real user-tap event — getUserMedia is allowed here.
  // We open the overlay immediately to stay in the gesture context,
  // then get media and begin signalling.
  if (_pc || _callDocId) { _teardown(); }
  _callType = type;
  _callRole = 'caller';
  _resetOverlay(currentFriendData, type);
  _openOverlay();
  _wireButtons();
  _startDotsAnim(type);
  _startCallFlow(type);
...
// ── Main call flow for CALLER ────────────────────────────
async function _startCallFlow(type) {
  try {
    // 1. Get media — must happen as close to the original tap as possible.
    //    On Safari iOS/iPadOS, the gesture context is preserved through one
    //    await if the Promise resolves quickly; getUserMedia is whitelisted.
    ...
    // 2. Show local preview for video calls
    if (type === 'video') {
      const lv = $('callLocalVideo');
      if (lv) { lv.srcObject = _localStream; _safePlay(lv); }
      const pip = $('callLocalPip');
    ...
    // 4. Reserve Firestore document ID upfront so ICE handler has it
    const callRef = doc(collection(db, 'calls'));
    _callDocId = callRef.id;

    // 5. ICE trickle — send candidates as they arrive
    _pc.onicecandidate = async ev => {
      if (ev.candidate && _callDocId) {
        await addDoc(
          collection(db, 'calls', _callDocId, 'callerCandidates'),
          ev.candidate.toJSON()
        ).catch(() => {});
      }
    };
    ...
    // 6. Remote tracks
    _pc.ontrack = ev => _handleRemoteTrack(ev);
    ...
    _pc.onconnectionstatechange = () => {
      if (_pc?.connectionState === 'disconnected') _hangup();
    };
    ...
    _callUnsub = onSnapshot(callRef, async snap => {
      ...
      if (data.answer && _pc.signalingState === 'have-local-offer') {
        ...
        // Subscribe to callee ICE candidates after remote description is set
        if (!_calleeCandUnsub) {
          _calleeCandUnsub = onSnapshot(
            collection(db, 'calls', _callDocId, 'calleeCandidates'),
            snap2 => {
              snap2.docChanges().forEach(async ch => {
                if (ch.type === 'added' && _pc) {
                  await _pc.addIceCandidate(new RTCIceCandidate(ch.doc.data())).catch(() => {});
                }
              });
            }
          );
      ...
      if (data.status === 'ended' && _callDocId === callRef.id) _hangup();
    });
    ...
    toast(
      err.name === 'NotAllowedError'
        ? 'Microphone / camera access denied. Check Settings → Safari → Microphone.'
...
// Called from a button tap — stays in gesture context for getUserMedia.
async function _answerCall(callerData, callId, callData) {
  if (_pc || _callDocId) await _teardown();

  _callType = callData.type;
  _callRole = 'callee';
  _callDocId = callId;
  currentFriendId   = callData.callerId;
  currentFriendData = callerData;

  _resetOverlay(callerData, callData.type);
  _openOverlay();
  _wireButtons();
  _setStatus(callData.type === 'video' ? '📹 Connecting…' : '📞 Connecting…');

  try {
    // 1. Get media — called inside tap handler, Safari is happy
    _localStream = await navigator.mediaDevices.getUserMedia({
      audio: {
        echoCancellation: true,
        noiseSuppression: true,
    ...
    // 7. Subscribe to caller ICE candidates BEFORE creating answer
    //    (they may arrive before our answer is ready)
    _callerCandUnsub = onSnapshot(
    ...
    // 9. Publish answer — triggers caller's snapshot
    ...
    // 10. Watch for hangup
    _callUnsub = onSnapshot(doc(db, 'calls', callId), snap => {
      const data = snap.data();
      if (data?.status === 'ended' && _callDocId === callId) _hangup();
    });
...
// Safari and Firefox differ wildly on event.streams[0].
// We always build our own MediaStream and add every track to it.
// Both the <audio> and <video> elements point to this one stream.
function _handleRemoteTrack(event) {
  _stopDots();

  // Build (or reuse) our shared remote stream
  if (!_remoteStream) {
    _remoteStream = (event.streams && event.streams[0])
      ? event.streams[0]
      : new MediaStream();
  }
  // If track wasn't attached to a stream, add it ourselves
  if (!event.streams || !event.streams[0]) {
    // Prevent duplicate tracks
    const existingIds = _remoteStream.getTracks().map(t => t.id);
    if (!existingIds.includes(event.track.id)) {
      _remoteStream.addTrack(event.track);
    }
  }

  // ── Audio element — always wire, handles both voice & video ──
  const audioEl = $('callRemoteAudio');
  if (audioEl && audioEl.srcObject !== _remoteStream) {
    audioEl.srcObject = _remoteStream;
  }
  // Always call play() — idempotent when already playing.
  ...
  if (_callType === 'video') {
    ...
    if (rv) {
      ...
      rv.style.display = '';
      _safePlay(rv);
      // Hide voice-center avatar once video is flowing
      const vc = $('callVoiceCenter');
      if (vc) vc.style.display = 'none';
    }
  }

  _setStatus(_callType === 'video' ? '📹 Connected' : '📞 Connected');
  _startTimer();
}

// ── Safe play helper ─────────────────────────────────────
// Safari sometimes needs a moment; we retry on failure.
function _safePlay(el) {
  const p = el.play();
  if (!p) return;
  p.catch(() => {
    // Autoplay blocked — attach one-time tap listener to resume
    const resume = () => {
      el.play().catch(() => {});
...
// ── Hang up & clean up everything ───────────────────────
async function _hangup() {
  _stopDots();
  _stopTimer();

  if (_callUnsub)       { _callUnsub();       _callUnsub       = null; }
  if (_callerCandUnsub) { _callerCandUnsub(); _callerCandUnsub = null; }
  if (_calleeCandUnsub) { _calleeCandUnsub(); _calleeCandUnsub = null; }

  if (_callDocId) {
    const id = _callDocId;
    _callDocId = null;
    updateDoc(doc(db, 'calls', id), { status: 'ended' }).catch(() => {});
  }
...
function _teardown() {
  ...
  if (_bgAnim) { _bgAnim(); _bgAnim = null; }
  _stopTimer();
  _stopDots();

  // Clear all media elements
  const els = ['callRemoteAudio', 'callRemoteVideo', 'callLocalVideo'];
  els.forEach(id => {
    const el = $(id);
    if (!el) return;
    el.srcObject = null;
    if (el.tagName === 'VIDEO') el.style.display = 'none';
  });
  const pip = $('callLocalPip');
  if (pip) { pip.style.display = 'none'; pip.style.left = ''; pip.style.top = ''; pip.style.right = '16px'; }

  _isMuted = false; _isCamOff = false;
  _callType = null; _callRole = null; _callDocId = null;
...
function _startDotsAnim(type) {
  let n = 0;
  _dotsInt = setInterval(() => {
    n = (n + 1) % 4;
    _setStatus((type === 'video' ? '📹 Video' : '📞 Voice') + ' calling' + '…'.repeat(n || 1));
  }, 600);
}
function _stopDots() {
  if (_dotsInt) { clearInterval(_dotsInt); _dotsInt = null; }
}

// ── Overlay open/close ───────────────────────────────────
function _openOverlay()  { const o = $('callOverlay'); if (o) o.style.display = 'flex'; }
function _closeOverlay() { const o = $('callOverlay'); if (o) o.style.display = 'none'; }

// ── Status text ──────────────────────────────────────────
function _setStatus(text) {
  [$('callType'), $('callTopStatus')].forEach(el => { if (el) el.textContent = text; });
}

// ── Animated background ──────────────────────────────────
function _startBgAnim() {
  const canvas = $('callBgCanvas');
  if (!canvas) return () => {};
  ...
  const draw = () => {
    ...
    g.addColorStop(0, 'rgba(255,107,53,0.06)');
    ...
    pts.forEach(p => {
      ...
      ctx.fillStyle = `rgba(255,190,11,${p.a})`;
      ctx.fill();
      p.x += p.dx; p.y += p.dy;
...
function _makePipDraggable() {
  ...
  const onMove = (cx, cy) => {
    ...
    ny = Math.max(70, Math.min(window.innerHeight - pip.offsetHeight - 100, ny));
    pip.style.left  = nx + 'px';
    pip.style.top   = ny + 'px';
    pip.style.right = 'auto';
  };
  const onUp = () => { dragging = false; pip.style.cursor = 'grab'; };
...
function _resetOverlay(peerData, type) {
  // Identity
  setAvatar($('callAvatar'), peerData);
  $('callName').textContent = peerData.displayName;
  _setStatus(type === 'video' ? '📹 Video calling…' : '📞 Voice calling…');

  // Timer hidden until connected
  const timerEl = $('callTimer');
  if (timerEl) { timerEl.style.display = 'none'; timerEl.textContent = '0:00'; }

  // Reset mute/cam state
  _isMuted = false; _isCamOff = false;
  _syncMuteBtn(); _syncCamBtn();

  // Camera button: show only for video calls
  const camItem = $('callCamCtrl');
  ...
  if (pip) {
    ...
    pip.style.left = ''; pip.style.top = ''; pip.style.right = '16px';
  }
  ...
  const ra = $('callRemoteAudio');
  if (ra) ra.srcObject = null;
  _remoteStream = null;

  // Voice center always visible at start (hidden once remote video connects)
  const vc = $('callVoiceCenter');
  ...
  if (_bgAnim) { _bgAnim(); _bgAnim = null; }
  _bgAnim = _startBgAnim();

  // Make PiP draggable
  _makePipDraggable();
}

// ── Mute / Camera button sync ────────────────────────────
function _syncMuteBtn() {
  const btn  = $('callMuteBtn');    if (!btn) return;
  const icon = $('callMuteIcon');
  const lbl  = $('callMuteLbl');
  btn.classList.toggle('call-btn-off', _isMuted);
  if (lbl) lbl.textContent = _isMuted ? 'Unmute' : 'Mute';
  if (icon) icon.innerHTML = _isMuted
    ? `<line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/>`
    : `<path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/>`;
}
function _syncCamBtn() {
  const btn = $('callCamBtn'); if (!btn) return;
  btn.classList.toggle('call-btn-off', _isCamOff);
  const lbl = $('callCamLbl'); if (lbl) lbl.textContent = _isCamOff ? 'Camera off' : 'Camera';
}

// ── Panel helpers ────────────────────────────────────────
function _showCallChat(show) {
  const p = $('callChatPanel');
  if (p) p.classList.toggle('call-chat-panel-hidden', !show);
}
function _showEmojiPicker(show) {
  const p = $('callEmojiPicker');
  if (p) p.classList.toggle('call-emoji-picker-hidden', !show);
}

// ── Wire overlay buttons ─────────────────────────────────
let _emojiPickerOutsideHandler = null;

function _wireButtons() {
  // End call
  const endBtn = $('callEndBtn');
  if (endBtn) endBtn.onclick = () => { _hangup(); toast('Call ended'); };
  ...
  if (camBtn) camBtn.onclick = () => {
    ...
    _syncCamBtn();
    toast(_isCamOff ? 'Camera off 📷' : 'Camera on 📹');
  };

  // Flip camera (mobile/iPad — switches front/back)
  const flipBtn = $('callFlipBtn');
  if (flipBtn) flipBtn.onclick = async () => {
    if (!_localStream || _callType !== 'video') return;
    const current = _localStream.getVideoTracks()[0];
    ...
    try {
      ...
      if (current) { current.stop(); _localStream.removeTrack(current); }
      _localStream.addTrack(newVideoTrack);
      const lv = $('callLocalVideo');
      if (lv) { lv.srcObject = null; lv.srcObject = _localStream; _safePlay(lv); }
      toast(newFacing === 'environment' ? 'Rear camera 📷' : 'Front camera 🤳');
    } catch (e) {
  ...
  // Emoji react
  const reactBtn = $('callReactBtn');
  if (reactBtn) reactBtn.onclick = e => {
    e.stopPropagation();
    const isHidden = $('callEmojiPicker')?.classList.contains('call-emoji-picker-hidden');
    _showEmojiPicker(isHidden);
  };
  document.querySelectorAll('.call-emoji-opt').forEach(btn => {
    btn.onclick = () => { _floatEmoji(btn.dataset.emoji); _showEmojiPicker(false); };
  });
  if (_emojiPickerOutsideHandler) document.removeEventListener('click', _emojiPickerOutsideHandler);
  _emojiPickerOutsideHandler = e => {
    if (!e.target.closest('#callEmojiPicker') && !e.target.closest('#callReactBtn')) {
      _showEmojiPicker(false);
    }
  };
  document.addEventListener('click', _emojiPickerOutsideHandler);

  // In-call chat
  const chatBtn = $('callChatBtn');
  if (chatBtn) chatBtn.onclick = () => {
    const isHidden = $('callChatPanel')?.classList.contains('call-chat-panel-hidden');
    _showCallChat(isHidden);
    if (isHidden) { const b = $('callChatBadge'); if (b) b.style.display = 'none'; }
  };
  const closeChatBtn = $('callCloseChatBtn');
  if (closeChatBtn) closeChatBtn.onclick = () => _showCallChat(false);
  ...
  const chatSend = () => {
    ...
    if (!txt) return;
    _appendCallMsg('You', txt);
    inp.value = '';
  };
  const sendBtn = $('callChatSendBtn');
  if (sendBtn) sendBtn.onclick = chatSend;
  const chatInp = $('callChatInput');
  if (chatInp) chatInp.onkeydown = e => { if (e.key === 'Enter') chatSend(); };
}
...
function _appendCallMsg(who, text) {
...
function listenForIncomingCalls() {
  ...
  onSnapshot(q, async snap => {
    for (const change of snap.docChanges()) {
      ...
      if (Date.now() - createdAt.getTime() > 60000) continue;
      const callerSnap = await getDoc(doc(db, 'users', data.callerId)).catch(() => null);
      if (!callerSnap?.exists()) continue;
      _showIncomingBanner(callerSnap.data(), change.doc.id, data);
    }
  });
}

function _showIncomingBanner(caller, callId, callData) {
  ...
  banner.innerHTML = `
    <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0;">
      ...
      <div style="min-width:0;">
        <div style="font-weight:600;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escHtml(caller.displayName)}</div>
        <div style="font-size:12px;color:var(--text-muted);margin-top:2px;">${callData.type === 'video' ? '📹 Video' : '📞 Voice'} call…</div>
      </div>
    </div>
    <button class="btn btn-primary  icb-accept"  style="padding:9px 18px;font-size:13px;flex-shrink:0;">Answer</button>
    <button class="btn btn-ghost    icb-decline" style="padding:9px 18px;font-size:13px;flex-shrink:0;color:#ff4757;">Decline</button>
  `;
  ...
  banner.querySelector('.icb-decline').addEventListener('click', async () => {
    clearTimeout(dismiss); banner.remove();
    await updateDoc(doc(db, 'calls', callId), { status: 'ended' }).catch(() => {});
  });

  // Answer tap — stays in gesture context for getUserMedia
  banner.querySelector('.icb-accept').addEventListener('click', () => {
    clearTimeout(dismiss); banner.remove();
    _answerCall(caller, callId, callData);
  });
}

// ── Profile card: call/video buttons ────────────────────
...
// ════════════════════════════════════════════════════════════
//  NEW FEATURE 1 — MESSAGE REACTIONS
//  Tap/long-press any message to add an emoji reaction.
//  Reactions are stored in Firestore and visible to both users.
// ════════════════════════════════════════════════════════════

const REACTION_EMOJIS = ['❤️','👍','😂','😮','😢','🔥','👏','💯'];

// Called from the context menu (right-click / long-press) in renderMessages()
window.__openReactionPicker = function(msgId, anchorEl) {
  document.querySelectorAll('.reaction-picker-popup').forEach(p => p.remove());

  const picker = document.createElement('div');
  picker.className = 'reaction-picker-popup';
  picker.innerHTML = REACTION_EMOJIS.map(e =>
    `<button class="rxn-opt" data-emoji="${e}" data-msgid="${msgId}">${e}</button>`
  ).join('');
  ...
  picker.querySelectorAll('.rxn-opt').forEach(btn => {
...
// ════════════════════════════════════════════════════════════
//  NEW FEATURE 2 — MESSAGE SEARCH HIGHLIGHT IN CHAT
//  Already partially implemented — we enhance it with a
//  jump-to-result button and match counter.
// ════════════════════════════════════════════════════════════

let _searchMatches  = [];
let _searchMatchIdx = 0;

window.__enhanceMessageSearch = function() {
  ...
  nav.id = 'searchNav';
  nav.style.cssText = 'display:none;align-items:center;gap:6px;margin-top:4px;font-size:12px;color:var(--text-muted);';
  nav.innerHTML = `
    <span id="searchCounter"></span>
    <button id="searchPrev" style="padding:2px 8px;border-radius:6px;border:1px solid var(--border);background:var(--bg-glass);color:var(--text-secondary);cursor:pointer;font-size:11px;">▲</button>
    <button id="searchNext" style="padding:2px 8px;border-radius:6px;border:1px solid var(--border);background:var(--bg-glass);color:var(--text-secondary);cursor:pointer;font-size:11px;">▼</button>
  `;
  wrap.appendChild(nav);

  const updateNav = () => {
    _searchMatches = Array.from(document.querySelectorAll('.search-match'));
    if (_searchMatches.length === 0) {
      nav.style.display = 'none';
      return;
    }
    nav.style.display = 'flex';
    $('searchCounter').textContent = `${_searchMatchIdx + 1} / ${_searchMatches.length}`;
    _searchMatches.forEach((el, i) => el.classList.toggle('search-match-focus', i === _searchMatchIdx));
    _searchMatches[_searchMatchIdx]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  };

  inp.addEventListener('input', () => {
    _searchMatchIdx = 0;
    setTimeout(updateNav, 50); // wait for handleMsgSearch to update classes
  });
...
// ════════════════════════════════════════════════════════════
//  NEW FEATURE 3 — MESSAGE DELIVERY RECEIPT & "SEEN" PULSE
//  Shows a subtle animated pulse on your own message when
//  the other user has opened the chat (i.e. read it).
//  Also adds a "last seen" glow to the conversation list item.
// ════════════════════════════════════════════════════════════

window.__applySeenPulse = function(msgEl, isSeen) {
  if (!msgEl) return;
  const tick = msgEl.querySelector('.msg-tick');
  if (!tick) return;
  if (isSeen) {
    tick.classList.add('msg-tick-seen-pulse');
    setTimeout(() => tick.classList.remove('msg-tick-seen-pulse'), 2000);
  }
};

// Inject styles for all 3 new features
(function _injectNewFeatureStyles() {
  const style = document.createElement('style');
  style.textContent = `

/* ══ NEW FEATURE 1: Reaction picker popup ══ */
...
.msg-reaction-badge:active { transform: scale(0.92); }
.msg-reaction-badge.mine { border-color: var(--accent); background: rgba(255,107,53,0.08); }
.msg-reaction-count { font-size: 11px; color: var(--text-muted); font-weight: 600; }

/* ══ NEW FEATURE 2: Search nav ══ */
.search-match-focus {
...
/* ══ NEW FEATURE 3: Seen pulse on tick ══ */
@keyframes seenPulse {
  0%   { text-shadow: 0 0 0 rgba(0,180,216,0); color: var(--accent); }
  50%  { text-shadow: 0 0 8px rgba(0,180,216,0.6); color: #00b4d8; }
  100% { text-shadow: 0 0 0 rgba(0,180,216,0); color: var(--accent); }
}
.msg-tick-seen-pulse {
  animation: seenPulse 1s ease 3;
}

/* ══ Call overlay — rebuilt from scratch ══ */
#callOverlay {
  display: none;
  position: fixed;
  inset: 0;
...
.call-btn-row + .call-btn-row { padding-bottom: 0; margin-bottom: 20px; }
.call-ctrl-item {
  display: flex;
  flex-direction: column;
  align-items: center;
...
.call-btn-secondary svg { width: 22px; height: 22px; }
.call-btn-secondary:active { transform: scale(0.91); }
.call-btn-secondary.call-btn-off {
...
.call-btn-end:active { transform: scale(0.88); background: #d63227; }
...
/* In-call chat */
.call-chat-panel-hidden { transform: translateX(100%); pointer-events: none; }
#callChatPanel {
  position: absolute;
...
.ccm-who { font-size: 11px; font-weight: 700; color: var(--accent); opacity: 0.9; }
.ccm-text { font-size: 13px; color: rgba(240,237,232,0.85); line-height: 1.5; word-break: break-word; }

/* Emoji picker */
.call-emoji-picker-hidden { opacity: 0; pointer-events: none; transform: translateX(-50%) translateY(8px); }
#callEmojiPicker {
  position: absolute;
  bottom: 190px;
  left: 50%;
...
.call-emoji-opt:active { transform: scale(1.4); }
...
function openProfileModal(userData, uid, isFriend) {
  ...
  const pronounsEl = $('profilePronouns');
  if (pronounsEl) {
    if (userData.pronouns) {
      pronounsEl.style.display = '';
      pronounsEl.textContent = userData.pronouns;
    } else {
  ...
  if (isFriend) {
    ...
    callBtn.addEventListener('click', () => {
      // Ensure friend state is set before initiating call
      currentFriendId   = uid;
      currentFriendData = userData;
      hideModal('profileModal');
      _initiateCall('voice');
...
$('editProfileBtn').addEventListener('click', () => {
  ...
  if ($('setupPronouns')) $('setupPronouns').value = currentUserData.pronouns || '';
  ...
  setTimeout(() => { window.syncLivePreview?.(); }, 50);
  showModal('setupModal');
});
