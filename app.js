...
function applyBetaUI() {
  $('betaToggleBtn').textContent = betaEnabled ? 'On ✓' : 'Off';
  $('betaToggleBtn').style.color = betaEnabled ? 'var(--accent)' : '';
  $('betaToggleBtn').style.borderColor = betaEnabled ? 'var(--border-accent)' : '';
  const item = document.getElementById('screenShareCtrlItem');
  if (item) item.style.display = betaEnabled ? '' : 'none';
}
applyBetaUI();
...
function setCallStatus(text) {
  if ($('callType'))      $('callType').textContent = text;
  if ($('callTopStatus')) $('callTopStatus').textContent = text;
}
...
// Animated particle background during voice calls
let bgAnimId = null;
...
function makePipDraggable() {
  ...
  const down = (cx, cy) => {
    drag = true; pip.style.cursor = 'grabbing';
  ...
  const up = () => { drag = false; pip.style.cursor = 'grab'; };
...
// Populate overlay with peer info
function populateOverlay(peerData, type) {
  ...
  if (camItem) camItem.style.display = type === 'video' ? '' : 'none';
  const pip = $('localVideoPip');
  if (pip) pip.style.display = 'none'; // hidden until connected

  // Reset mute/cam buttons
  isMuted = false; isCamOff = false;
  resetMuteBtn(); resetCamBtn();

  // Hide remote video until connected
  ...
  // Close side panels
  $('callChatPanel').style.display = 'none';
  const picker = $('callEmojiPicker');
...
async function startCall(type) {
  callType = type;
  populateOverlay(currentFriendData, type);
  openCallOverlay();
  ...
  try {
    // Get media — audio ALWAYS, video only for video calls
    const constraints = {
      audio: true,
      ...
      if (data?.answer && peerConnection && peerConnection.signalingState !== 'stable') {
        ...
        onSnapshot(collection(db, 'calls', callDocId, 'calleeCandidates'), (s) => {
          s.docChanges().forEach(async ch => {
            if (ch.type === 'added') await peerConnection?.addIceCandidate(new RTCIceCandidate(ch.doc.data())).catch(() => {});
          });
        });
      }
    ...
    if (err.name === 'NotAllowedError' || err.name === 'NotFoundError') {
      toast('Mic/camera access denied', 'error');
...
async function answerCall(caller, callId, callData) {
  ...
  populateOverlay(caller, callData.type);
  openCallOverlay();
...
function wireCallButtons(dotInterval) {
  ...
  // Speaker (visual only on web — no browser API to force speaker)
  const spkBtn = $('speakerBtn');
  if (spkBtn) {
    spkBtn.onclick = () => {
      isSpeakerOn = !isSpeakerOn;
      spkBtn.classList.toggle('call-btn-active', isSpeakerOn);
  ...
  if (reactBtn && emojiPicker) {
    reactBtn.onclick = (e) => {
      e.stopPropagation();
      const showing = emojiPicker.style.display !== 'none' && emojiPicker.style.display !== '';
      emojiPicker.style.display = showing ? 'none' : 'flex';
    };
    emojiPicker.querySelectorAll('.call-emoji-opt').forEach(btn => {
      btn.onclick = () => {
        sendFloatEmoji(btn.dataset.emoji);
  ...
  if (chatBtn && chatPanel) {
    chatBtn.onclick = () => {
      ...
      if (!open) {
        const badge = $('callChatBadge');
        if (badge) badge.style.display = 'none';
      }
    };
    $('closeCallChatBtn').onclick = () => { chatPanel.style.display = 'none'; };
    const sendChat = () => {
    ...
    $('callChatSendBtn').onclick = sendChat;
    $('callChatInput').onkeydown = (e) => { if (e.key === 'Enter') sendChat(); };
  }
  ...
  document.addEventListener('click', (e) => {
    const picker = $('callEmojiPicker');
    if (picker && !e.target.closest('#callEmojiPicker') && !e.target.closest('#callReactBtn')) {
...
function appendCallChatMsg(who, text) {
  ...
  box.scrollTop = 99999;
  // Badge if closed
  const panel = $('callChatPanel');
  if (panel && panel.style.display !== 'flex' && who !== 'You') {
    const badge = $('callChatBadge');
...
async function beginScreenShare() {
  try {
    ...
    // Show screen video locally
    const sv = $('screenShareVideo');
    sv.srcObject = screenStream;
    sv.style.display = '';

    // Show indicator
    const ind = $('screenShareIndicator');
    if (ind) ind.style.display = 'flex';

    isScreenSharing = true;
    const ssBtn = $('screenShareBtn');
    if (ssBtn) ssBtn.classList.add('call-btn-active');
...
async function stopScreenShare() {
  ...
  const ssBtn = $('screenShareBtn');
  if (ssBtn) ssBtn.classList.remove('call-btn-active');
