...
function showChatPanel(show) {
  ...
  if (!p) return;
  if (show) { p.classList.remove('call-chat-panel-hidden'); }
  else      { p.classList.add('call-chat-panel-hidden'); }
}

function showEmojiPicker(show) {
  const p = $('callEmojiPicker');
  if (!p) return;
  if (show) { p.classList.remove('call-emoji-picker-hidden'); }
  else      { p.classList.add('call-emoji-picker-hidden'); }
}
...
function resetOverlay(peerData, type) {
  ...
  // Camera controls only for video
  const camItem = $('cameraCtrlItem'); if (camItem) camItem.style.display = type==='video' ? 'flex':'none';
  // PiP hidden until we have stream
  const pip = $('localVideoPip'); if (pip) pip.style.display = 'none';
  // Hide remote feeds
  const rv = $('remoteVideo'); if (rv) rv.style.display = 'none';
  const sv = $('screenShareVideo'); if (sv) sv.style.display = 'none';
...
// ── KEY FIX: Route remote tracks correctly ──
// Route remote tracks. Audio → dedicated <audio> element. Video → <video> element.
// Using separate audio element ensures voice calls work even with no video element shown.
function handleRemoteTrack(event, type) {
  const stream = event.streams[0];
  if (!stream) return;

  // Always pipe the full stream to the audio element so voice is heard
...
async function startCall(type) {
  ...
  try {
    ...
    peerConnection.ontrack = (ev) => { clearInterval(iv); handleRemoteTrack(ev, type); };
    ...
    await setDoc(callRef, { offer:{type:offer.type,sdp:offer.sdp}, callerId:currentUser.uid, calleeId:currentFriendId, type, status:'calling', createdAt:serverTimestamp() });
    ...
    callUnsub = onSnapshot(callRef, async snap => {
      ...
      if (data?.answer && peerConnection && peerConnection.signalingState !== 'stable') {
        ...
        onSnapshot(collection(db,'calls',callDocId,'calleeCandidates'), s => {
          s.docChanges().forEach(async ch => { if(ch.type==='added') await peerConnection?.addIceCandidate(new RTCIceCandidate(ch.doc.data())).catch(()=>{}); });
        });
      }
...
async function answerCall(caller, callId, callData) {
  ...
  try {
    ...
    callUnsub = onSnapshot(doc(db,'calls',callId), snap => { if(snap.data()?.status==='ended') endCall(); });
...
function wireCallButtons(dotInterval) {
  $('endCallBtn').onclick = () => { if(dotInterval)clearInterval(dotInterval); endCall(); toast('Call ended'); };
  ...
  // Chat — FIX: use class toggling, not display string comparison
  $('callChatBtn').onclick = () => {
  ...
  $('callChatSendBtn').onclick = sendMsg;
  $('callChatInput').onkeydown = e => { if(e.key==='Enter') sendMsg(); };
...
function resetMuteBtn() {
  const btn=$('muteBtn'), icon=$('muteBtnIcon'), lbl=$('muteLbl');
  if (isMuted) {
    btn.classList.add('call-btn-off');
    ...
    btn.classList.remove('call-btn-off');
...
// ── SCREEN SHARE (BETA) ──
// FIX: For voice calls (no video sender), add screen track directly.
// For video calls, replace the existing video sender track.
  ...
  try {
    ...
    if (peerConnection) {
      ...
      if (videoSender) {
        ...
        // Voice call — add new video track for screen share
        peerConnection.addTrack(screenTrack, screenStream);
      }
    }

    // Show locally
    const sv=$('screenShareVideo'); sv.srcObject=screenStream; sv.style.display='';
...
async function endCall() {
  ...
  isMuted=false; isCamOff=false; isScreenSharing=false; callStartedAt=null;
  closeCallOverlay();
}
