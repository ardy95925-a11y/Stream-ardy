...
async function saveProfile() {
  ...
  const cardStyle   = document.querySelector('.card-style-btn.active')?.dataset.style || 'flame';
  ...
  try {
    ...
    const userData = {
      uid: currentUser.uid, displayName, username, photoURL, bio, cardStyle,
...
async function showCallModal(type) {
  ...
  if (videoArea) videoArea.style.display = type === 'video' ? 'flex' : 'none';
  ...
  try {
    ...
    // Notify friend via chat
    await sendMessage({ type: 'text', text: `📞 ${type === 'video' ? 'Video' : 'Voice'} call — missed call (in progress)` }).catch(() => {});
    ...
    clearInterval(iv);
    // If no camera/mic permission, still show the UI with simulated call
    if (err.name === 'NotAllowedError' || err.name === 'NotFoundError') {
      toast('Camera/microphone access denied — showing call UI only', 'error');
      ...
      if (localStream) {
        ...
        if (audioTrack) {
          ...
          muteBtn.innerHTML = audioTrack.enabled
            ...
            : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:#ff4757"><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>`;
...
async function answerCall(caller, callId, callData) {
  ...
  const videoArea = $('callVideoArea');
  if (videoArea) videoArea.style.display = callData.type === 'video' ? 'flex' : 'none';
...
function startProfileParticles(style) {
  ...
  $('closeProfileModal').addEventListener('click', stop, { once: true });
}
