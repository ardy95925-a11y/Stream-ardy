...
function syncLivePreview() {
  ...
  const tagEl = document.getElementById('livePreviewTag');
  const bioEl = document.getElementById('livePreviewBio');
  const statusEl = document.getElementById('livePreviewStatus');
  const pronounsEl = document.getElementById('livePreviewPronouns');

  const name = document.getElementById('setupDisplayName')?.value || 'Your Name';
  const username = document.getElementById('setupUsername')?.value || 'username';
  const bio = document.getElementById('setupBio')?.value || '';
  const status = document.getElementById('setupStatusMsg')?.value || '';
  const mood = document.getElementById('setupMoodEmoji')?.value || '💬';
  const pronouns = document.getElementById('setupPronouns')?.value || '';
  ...
  if (pronounsEl) {
    ...
    pronounsEl.style.display = pronouns ? '' : 'none';
  }
  if (statusEl) {
    if (status) {
...
function setupProfileCardPreview() {
  ...
  ['setupDisplayName','setupUsername','setupBio','setupStatusMsg','setupPronouns'].forEach(id => {
    document.getElementById(id)?.addEventListener('input', syncLivePreview);
  });
  document.getElementById('setupMoodEmoji')?.addEventListener('change', syncLivePreview);

  // Sync photo
  const pfpPreview = document.getElementById('pfpPreview');
  const pfpObserver = new MutationObserver(() => {
    const img = pfpPreview?.querySelector('img');
    const av = document.getElementById('livePreviewAvatar');
    if (av) {
      if (img) {
        av.innerHTML = `<img src="${img.src}" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
      } else {
  ...
  document.querySelectorAll('.card-style-btn:not(.bubble-shape-btn)').forEach(btn => {
    btn.addEventListener('click', () => {
      const header = document.getElementById('livePreviewHeader');
      if (header) header.dataset.style = btn.dataset.style;
      startMiniParticles(btn.dataset.style);
    });
  });

  // Sync bubble shape to preview bubbles
  document.querySelectorAll('.bubble-shape-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      // bubble shape is applied globally by app.js applyBubbleShape
    });
  });
...
function startMiniParticles(style) {
  ...
  const canvas = document.getElementById('livePreviewCanvas') || document.getElementById('miniParticles');
  if (!canvas) return;
...
function setupProfileCardStatusMessage() {
  // Fields now exist in HTML directly — just wire up live preview sync
  const statusInput = document.getElementById('setupStatusMsg');
  const moodSelect = document.getElementById('setupMoodEmoji');
  if (statusInput) {
    statusInput.addEventListener('input', syncLivePreview);
  }
  if (moodSelect) {
    moodSelect.addEventListener('change', syncLivePreview);
  }
}
