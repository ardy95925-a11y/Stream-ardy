...
// ── WebRTC Call System (Full-Screen) ──────────────────
...
let betaFeaturesEnabled = JSON.parse(localStorage.getItem('betaFeatures') || 'false');

const ICE_SERVERS = { iceServers: [{ urls: 'stun:stun.l.google.com:19302' }, { urls: 'stun:stun1.l.google.com:19302' }] };

// Beta features setting
$('betaFeaturesBtn').textContent = betaFeaturesEnabled ? 'On ✓' : 'Off';
$('betaFeaturesBtn').addEventListener('click', () => {
  betaFeaturesEnabled = !betaFeaturesEnabled;
  localStorage.setItem('betaFeatures', JSON.stringify(betaFeaturesEnabled));
  $('betaFeaturesBtn').textContent = betaFeaturesEnabled ? 'On ✓' : 'Off';
  toast(betaFeaturesEnabled ? 'Beta features enabled! 🎉' : 'Beta features disabled', betaFeaturesEnabled ? 'success' : '');
  updateBetaUI();
});

function updateBetaUI() {
  const item = $('screenShareCtrlItem');
  if (item) item.style.display = betaFeaturesEnabled ? '' : 'none';
}

// ── Animated canvas background for voice calls ──
function startCallBackground() {
  ...
  function draw() {
    ...
    particles.forEach(p => {
      ...
      ctx.fillStyle = `rgba(255,190,11,${p.opacity})`;
      ctx.fill();
      p.x += p.dx; p.y += p.dy;
...
// ── Draggable PiP ──
function makeDraggablePip() {
  const wrap = $('callLocalWrap');
  if (!wrap) return;
  ...
  const moveDrag = (cx, cy) => {
    ...
    const pw = window.innerWidth, ph = window.innerHeight, wr = wrap.offsetWidth, wh = wrap.offsetHeight;
    nx = Math.max(8, Math.min(pw - wr - 8, nx));
    ny = Math.max(70, Math.min(ph - wh - 120, ny));
...
// ── Open call overlay ──
function openCallOverlay(peerData, type) {
  const overlay = $('callOverlay');
  overlay.classList.remove('hidden');
  // Populate names
  setAvatar($('callAvatar'), peerData);
  $('callName').textContent = peerData.displayName;
  $('callTopbarName').textContent = peerData.displayName;
  $('callType').textContent = (type === 'video' ? '📹 Video' : '📞 Voice') + ' calling…';
  $('callTopbarStatus').textContent = 'Calling…';
  $('callDuration').style.display = 'none';
  // Show/hide video controls
  $('videoToggleItem').style.display = type === 'video' ? '' : 'none';
  $('callLocalWrap').style.display = type === 'video' ? '' : 'none';
  // Voice call: show bg & avatar; hide remote video
  $('callVoiceCenter').style.display = '';
  $('remoteVideo').style.display = 'none';
  // Start background
  stopCallBg = startCallBackground();
  // Beta UI
  updateBetaUI();
  // Draggable PiP
  makeDraggablePip();
  ...
  $('callEmojiPicker').classList.add('hidden');
}

async function showCallModal(type) {
  callType = type;
  openCallOverlay(currentFriendData, type);
  ...
  const iv = setInterval(() => {
    dots = (dots + 1) % 4;
    const base = type === 'video' ? '📹 Video' : '📞 Voice';
    $('callType').textContent = base + ' calling' + '.'.repeat(dots);
    $('callTopbarStatus').textContent = 'Calling' + '.'.repeat(dots);
  }, 600);
  ...
  try {
    ...
    peerConnection.ontrack = (event) => {
      ...
      $('callType').textContent = type === 'video' ? '📹 Connected' : '📞 Connected';
      $('callTopbarStatus').textContent = 'Connected';
      startDurationTimer();
    };
    ...
    peerConnection.oniceconnectionstatechange = () => {
      ...
      if (s === 'connected') { $('callTopbarStatus').textContent = 'Connected'; }
    };
    ...
    peerConnection.onicecandidate = async (event) => {
      if (event.candidate) await addDoc(collection(db, 'calls', callDocId, 'callerCandidates'), event.candidate.toJSON()).catch(() => {});
    };
    ...
    callUnsub = onSnapshot(callRef, async (snap) => {
      ...
      if (data?.answer && peerConnection.signalingState !== 'stable') {
        clearInterval(iv);
        await peerConnection.setRemoteDescription(new RTCSessionDescription(data.answer)).catch(() => {});
        $('callType').textContent = type === 'video' ? '📹 Connected' : '📞 Connected';
        $('callTopbarStatus').textContent = 'Connected';
        onSnapshot(collection(db, 'calls', callDocId, 'calleeCandidates'), (s) => {
          s.docChanges().forEach(async change => {
            if (change.type === 'added') await peerConnection.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(() => {});
          });
        });
      }
      if (data?.status === 'ended') { clearInterval(iv); endCall(); }
    });
    ...
    if (err.name === 'NotAllowedError' || err.name === 'NotFoundError') {
      ...
      $('callType').textContent = (type === 'video' ? '📹 Video' : '📞 Voice') + ' call (no mic/cam)';
    } else {
      toast('Could not start call: ' + err.message, 'error');
    }
  }

  setupCallControls(iv);
}

function setupCallControls(iv) {
  // End call
  $('endCallBtn').onclick = () => { if (iv) clearInterval(iv); endCall(); toast('Call ended'); };
  ...
  muteBtn.onclick = () => {
    if (localStream) {
      ...
      if (track) {
        ...
        muteBtn.title = isMuted ? 'Unmute' : 'Mute mic';
        muteBtn.innerHTML = isMuted
          ? `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:26px;height:26px"><line x1="1" y1="1" x2="23" y2="23"/><path d="M9 9v3a3 3 0 0 0 5.12 2.12M15 9.34V4a3 3 0 0 0-5.94-.6"/><path d="M17 16.95A7 7 0 0 1 5 12v-2m14 0v2a7 7 0 0 1-.11 1.23"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>`
          : `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:26px;height:26px"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>`;
        document.querySelector('.call-ctrl-label:has(#muteBtn)')?.setAttribute('data-label', isMuted ? 'Unmute' : 'Mute');
    ...
    videoToggleBtn.onclick = () => {
      if (localStream) {
        ...
        if (track) {
          ...
          videoToggleBtn.classList.toggle('off', isCameraOff);
          videoToggleBtn.title = isCameraOff ? 'Turn camera on' : 'Turn camera off';
  ...
  if (flipBtn) {
    flipBtn.onclick = async () => {
      ...
      } catch(err) { toast('Could not flip camera', 'error'); }
    };
  }
  ...
  if (screenShareBtn) {
    screenShareBtn.onclick = async () => {
      if (!betaFeaturesEnabled) { toast('Enable beta features in Settings first', 'error'); return; }
      if (isScreenSharing) {
  ...
  const emojiPicker = $('callEmojiPicker');
  if (reactBtn && emojiPicker) {
    reactBtn.onclick = (e) => { e.stopPropagation(); emojiPicker.classList.toggle('hidden'); };
    emojiPicker.querySelectorAll('.call-emoji-opt').forEach(btn => {
      btn.onclick = (e) => {
        e.stopPropagation();
  ...
  const chatPanel = $('callChatPanel');
  if (chatBtn && chatPanel) {
    chatBtn.onclick = () => {
      chatPanel.classList.toggle('hidden');
      $('callChatBadge').classList.add('hidden');
    };
    $('closeCallChat').onclick = () => chatPanel.classList.add('hidden');
    const chatSend = $('callChatSend');
    const chatInput = $('callChatInput');
    const sendChat = () => {
      const txt = chatInput.value.trim();
      if (!txt) return;
      const msg = { text: txt, sender: 'You', time: new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}) };
      addCallChatMsg(msg);
      chatInput.value = '';
    };
  ...
  // Quality info
  const qualBtn = $('callQualityBtn');
  if (qualBtn) {
    qualBtn.onclick = async () => {
      if (!peerConnection) return;
      const stats = await peerConnection.getStats().catch(() => null);
      if (!stats) { toast('No connection stats available'); return; }
      let info = 'Connection: ';
      stats.forEach(s => {
        if (s.type === 'candidate-pair' && s.state === 'succeeded') {
          info += `RTT ${s.currentRoundTripTime ? Math.round(s.currentRoundTripTime * 1000) + 'ms' : 'unknown'}`;
        }
      });
      toast(info);
    };
...
function addCallChatMsg(msg) {
  callChatMessages.push(msg);
  const el = document.createElement('div');
  el.className = 'call-chat-msg';
  el.innerHTML = `<div class="ccm-name">${escHtml(msg.sender)} · ${msg.time}</div><div class="ccm-text">${escHtml(msg.text)}</div>`;
  $('callChatMessages').appendChild(el);
  $('callChatMessages').scrollTop = 9999;
  // Badge if panel closed
  if ($('callChatPanel').classList.contains('hidden') && msg.sender !== 'You') {
    $('callChatBadge').classList.remove('hidden');
  }
}

function sendCallReaction(emoji) {
  const layer = $('callReactionsLayer');
  const el = document.createElement('div');
...
async function startScreenShare() {
  try {
    ...
    // Show indicator
    let ind = document.querySelector('.screenshare-indicator');
    if (!ind) {
      ind = document.createElement('div');
      ind.className = 'screenshare-indicator';
      ind.innerHTML = '<div class="screenshare-dot"></div> Sharing screen';
      $('callOverlay').appendChild(ind);
    }
    ...
    if (err.name !== 'NotAllowedError') toast('Could not share screen: ' + err.message, 'error');
  }
}
...
async function stopScreenShare() {
  ...
  const sv = $('screenShareVideo');
  sv.srcObject = null; sv.classList.add('hidden');
  $('remoteVideo').style.opacity = '';
  document.querySelector('.screenshare-indicator')?.remove();
...
async function endCall() {
  ...
  if (screenStream) { screenStream.getTracks().forEach(t => t.stop()); screenStream = null; }
  if (stopCallBg) { stopCallBg(); stopCallBg = null; }
  if (callDurationInterval) { clearInterval(callDurationInterval); callDurationInterval = null; }
  isScreenSharing = false; isMuted = false; isCameraOff = false;
  $('localVideo').srcObject = null;
  ...
  $('callChatPanel').classList.add('hidden');
  $('callEmojiPicker').classList.add('hidden');
  $('callReactionsLayer').innerHTML = '';
  // Reset buttons
  $('muteBtn').classList.remove('off');
  $('muteBtn').innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:26px;height:26px"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>`;
  const vtb = $('videoToggleBtn');
  if (vtb) { vtb.classList.remove('off'); }
  ...
  onSnapshot(q, async (snap) => {
    for (const change of snap.docChanges()) {
      if (change.type === 'added') {
        ...
        showIncomingCallUI(caller, change.doc.id, data);
      }
    }
  });
}
...
function showIncomingCallUI(caller, callId, callData) {
  ...
  const avEl = document.createElement('div'); avEl.className = 'avatar md'; setAvatar(avEl, caller);
  banner.innerHTML = `
...
async function answerCall(caller, callId, callData) {
  ...
  openCallOverlay(caller, callData.type);
  $('callType').textContent = callData.type === 'video' ? '📹 Connected' : '📞 Connected';
  $('callTopbarStatus').textContent = 'Connected';
  ...
  try {
    ...
    const localVideo = $('localVideo');
    const remoteVideo = $('remoteVideo');
    if (localVideo && callData.type === 'video') localVideo.srcObject = localStream;
    ...
    peerConnection.onicecandidate = async (event) => {
      if (event.candidate) await addDoc(collection(db, 'calls', callId, 'calleeCandidates'), event.candidate.toJSON()).catch(() => {});
    };

    callDocId = callId;
    await peerConnection.setRemoteDescription(new RTCSessionDescription(callData.offer));

    onSnapshot(collection(db, 'calls', callId, 'callerCandidates'), (s) => {
      s.docChanges().forEach(async change => {
        if (change.type === 'added') await peerConnection.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(() => {});
      });
    });
    ...
    await updateDoc(doc(db, 'calls', callId), { answer: { type: answer.type, sdp: answer.sdp }, status: 'connected' });
