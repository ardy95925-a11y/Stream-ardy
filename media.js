...
.multi-file-send-btn:hover { transform: scale(1.08); }
  ...
  // We hook into the existing infrastructure via a global bridge
  function getSendFn() {
    return window.__pulseSendMessage;
  }
  ...
  function waitForSendBridge() {
    if (window.__pulseSendMessage) return;

    // Intercept window.sendMessage if app.js exports it
    // We poll briefly to detect when it's ready
    let attempts = 0;
    const iv = setInterval(() => {
      attempts++;
      // If app.js has loaded and set it up, we're good
      if (window.__pulseSendMessage || attempts > 100) clearInterval(iv);
    }, 100);
  }

  // In app.js, add this after sendMessage function:
  // window.__pulseSendMessage = sendMessage;
  // We inject a small patch via a script tag if app.js hasn't done it
  function injectSendBridgePatch() {
    // Check if already bridged
    if (window.__pulseSendMessagePatched) return;
    window.__pulseSendMessagePatched = true;

    // Use MutationObserver to detect when Firebase/auth loads and app.js fires
    // We proxy document to catch the moment sendMessage becomes available
    const origOpen = XMLHttpRequest.prototype.open;

    // Simpler approach: override the global after a small delay
    // by checking for the known app.js global state
    function tryBridge() {
      // app.js sets currentUser after auth — we use that as a signal
      // Then we create our own sendMessage wrapper using the existing Firestore calls
      if (typeof window.__pulseSendMessage === 'undefined') {
        // Create a stub that will be replaced once app.js fires initApp
        window.__pulseSendMessage = function() {
          showToast('Open a chat first to send media', 'error');
        };
      }
    }
